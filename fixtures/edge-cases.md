...
`gov_grants`, `tax_transparency`, `rd_tax_incentive`, `asic_afs_rep`, `asic_credit_rep`,
`wgea_reporter`, `asic_smsf_auditor`) so the join seam is exercised end-to-end (not just
stubbed). All ABNs are checksum-valid (mod-89). Each row exercises a distinct edge case
the flatten + schema + verify must handle.
...
| 51000000810 | GrantConnect (govGrants) | Recipient of **2** grants, summed value `450000.75` (fractional cents), 2019→2023                           | `govGrants{}` populated; `flags.receivesGovGrants=true`; multi-grant sum + date range |
| 51000000923 | GrantConnect (govGrants) | Recipient of **1** grant `75000.00` (co-exists with `charity{}` + `dgr[]`)                                  | `govGrants{}` single-grant; recipient signal distinct from `govSpend` supplier signal |
