...
| 51000000761 | ASIC AFS Licensee    | AFS licence on a private company ABN                                                                    | `financialServicesLicence{}` populated (1:0..1 on ABN)                          |
| 51000000793 | ASIC Credit Licensee | Credit licence on a public company ABN (`APPR`)                                                         | `creditLicence{}` populated; raw status code passed through                     |
| 51000000987 | ASIC Banned & Disq.  | **Two** banning actions, joined via **ACN** (`asic_number='000000987'`), one permanent (null `endDate`) | `bannedDisqualified[]` fan-out (1:N via ACN, not ABN) + permanent-ban null date |
