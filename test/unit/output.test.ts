...
describe("runOutput", () => {
  it("splits per state, gzips each, and writes metadata with attribution", async () => {
    ...
    // metadata.json: counts + all fourteen CC-BY sources + an extract date
    ...
    expect(meta.sources).toHaveLength(14);
  ...
  it("itemizes all fourteen datasets with a CC-BY attribution", () => {
    expect(ABN_SOURCES).toHaveLength(14);
    ...
    expect(new Set(ABN_SOURCES.map((s) => s.name)).size).toBe(14);
    expect(new Set(ABN_SOURCES.map((s) => s.url)).size).toBe(14);
  });
